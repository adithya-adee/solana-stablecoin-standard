...
import { ROLE_ID_MAP, TIER_ORDINAL_MAP, ORDINAL_TO_TIER_MAP, StablecoinTiers } from '../src/types';
...
describe('Type mappings', () => {
  ...
  describe('ORDINAL_TO_TIER_MAP', () => {
    it('maps u8 back to preset string', () => {
      expect(ORDINAL_TO_TIER_MAP[1]).toBe('sss-1');
      expect(ORDINAL_TO_TIER_MAP[2]).toBe('sss-2');
      expect(ORDINAL_TO_TIER_MAP[3]).toBe('sss-3');
    });

    it('is consistent with TIER_ORDINAL_MAP', () => {
      for (const [key, value] of Object.entries(TIER_ORDINAL_MAP)) {
        expect(ORDINAL_TO_TIER_MAP[value]).toBe(key);
      }
    });
  });
