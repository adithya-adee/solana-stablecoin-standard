...
async function main() {
  ...
  const hookProgram = new Program<SssTransferHook>(SssTransferHookIdl as SssTransferHook, provider);
  ...
  logEntry(
    'Final balance',
    `${(finalBalance / 1e9).toFixed(4)} SOL (used ${((balance - finalBalance) / 1e9).toFixed(4)} SOL)`,
    icons.info,
  );
  logHeader('Done');
}
