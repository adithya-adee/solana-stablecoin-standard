...
async function main() {
  ...
  const { publicKey: auditorPubkey } = generateTestElGamalKeypair();
  ...
  try {
    txSigs.configureAccount = await sss.confidential.configureAccount(ata, elGamalPubkey, aesKey);
    logSuccess(`Account configured. Tx: ${txSigs.configureAccount}`);
  } catch (err: unknown) {
    const msg = err instanceof Error ? err.message : String(err);
    logWarning(`ConfigureAccount skipped (requires Rust ZKP service): ${msg.split('\n')[0]}`);
    notes.push(
      'ConfigureAccount requires a PubkeyValidity ZK proof which can only be generated by the solana-zk-sdk Rust crate',
    );
    txSigs.configureAccount = 'skipped-needs-rust-zkp';
  }
